...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
    ...
    @Query("SELECT i FROM Invoice i WHERE i.deletedAt IS NULL AND (i.customer.carPlate LIKE %:query% OR i.customer.phone LIKE %:query%) ORDER BY i.invoiceNumber DESC")
    List<Invoice> searchByCustomerCarPlateOrPhone(@Param("query") String query);

    @Query("SELECT i FROM Invoice i WHERE i.deletedAt IS NULL ORDER BY i.invoiceNumber DESC")
    List<Invoice> findAllActiveOrderByDateDesc();

    @Query("SELECT i FROM Invoice i ORDER BY i.invoiceNumber DESC")
    List<Invoice> findAllIncludeDeletedOrderByDateDesc();

    @Query("SELECT i FROM Invoice i WHERE (i.customer.carPlate LIKE %:query% OR i.customer.phone LIKE %:query%) ORDER BY i.invoiceNumber DESC")
